...
parallel lateral (:math:`x` and :math:`y`) boundaries are identical. This
means that a flow leaving through one side reappears on the opposite side.
...
(horizontal) velocities parallel to the boundaries may either be fixed to 0 (no
slip), or free to attain other values (free slip).
